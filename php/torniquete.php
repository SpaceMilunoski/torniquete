...
<?php
    ...
    class Torniquete extends Conexion {//esta clase hereda de Conexion
        ...
        public function Validar($valormatricula){//con esta funcion se valida que este registrada la persona a ingresar
          ...
          $this->resultado = $this->con->query("Select library.validarCliente('".$this->matricula."');");//mandamos llamar a la funcion en la bd que verifica si el cliente existe
          ...
          if($this->fila[0]!=false){//validamos si el resultado  que nos dio la bd es verdadero 
              $this->resultado = $this->con->query("Select library.obtenerNombre('".$this->matricula."');");//obtenemos el nombre de el usuario
